...
const { mutate: updateStatus, isPending } = useUpdateLoanStatus();
...
const handleAction = (loan: any, action: "APPROVE" | "REJECT" | "RETURN") => {
...
const confirmAction = () => {
    ...
    updateStatus({ id: selectedLoan.value._id, status: newStatus }, {
        onSuccess: () => {
            addToast({ title: "Thành công", description: "Đã cập nhật trạng thái phiếu mượn.", variant: "success" });
            showConfirm.value = false;
            selectedLoan.value = null;
        },
        onError: () => {
            addToast({ title: "Lỗi", description: "Không thể cập nhật trạng thái.", variant: "error" });
        }
    });
};
...
const getStatusBadge = (status: string) => {
    switch (status) {
        case "DANG_CHO": return { class: "bg-yellow-100 text-yellow-800 border-yellow-300", label: "Chờ duyệt", icon: Clock };
        case "DANG_MUON": return { class: "bg-blue-100 text-blue-800 border-blue-300", label: "Đang mượn", icon: CheckCircle };
        case "DA_TRA": return { class: "bg-green-100 text-green-800 border-green-300", label: "Đã trả", icon: RotateCcw };
        case "DA_TU_CHOI": return { class: "bg-red-100 text-red-800 border-red-300", label: "Đã từ chối", icon: XCircle };
        default: return { class: "bg-gray-100", label: status, icon: MoreHorizontal };
    }
};
</script>
...
<template>
    <div class="space-y-6">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-purple-200 border-b-4 border-black text-black">
                        <th class="p-4 border-r-2 border-black font-black uppercase w-28">Mã Phiếu</th>
                        ...
                        <th class="p-4 border-r-2 border-black font-black uppercase w-32">Ngày Mượn</th>
                        <th class="p-4 border-r-2 border-black font-black uppercase w-40 text-center">Trạng Thái</th>
                ...
                <tbody class="divide-y-2 divide-black">
                    ...
                    <tr v-else-if="filteredLoans.length === 0">
                        <td colspan="6" class="p-8 text-center text-gray-500 font-bold">Không tìm thấy phiếu mượn nào.</td>
                    </tr>
                    <tr v-for="loan in filteredLoans" :key="loan._id" class="hover:bg-purple-50 transition-colors">
                        <td class="p-4 border-r-2 border-black font-mono font-bold">{{ loan.maPhieuMuon }}</td>
                        <td class="p-4 border-r-2 border-black">
                            <p class="font-bold">{{ loan.docGia.hoLot }} {{ loan.docGia.ten }}</p>
                            <p class="text-xs text-gray-500 font-mono">{{ loan.docGia.maDocGia }}</p>
                        </td>
                        <td class="p-4 border-r-2 border-black">
                            <p class="font-bold text-sm">{{ loan.banSao.sach.tenSach }}</p>
                            <p class="text-xs text-gray-500 font-mono">Bản sao: {{ loan.banSao.maBanSao }}</p>
                        </td>
                        <td class="p-4 border-r-2 border-black font-medium text-sm">
                            {{ formatDate(loan.ngayMuon) }}
                            <div v-if="loan.ngayTra" class="text-xs text-green-600 mt-1">Trả: {{ formatDate(loan.ngayTra) }}</div>
                        </td>
                            ...
                            <div class="flex justify-center gap-2">
                                <template v-if="loan.trangThai === 'DANG_CHO'">
                                    <button 
                                        ...
                                        class="p-2 bg-green-400 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none transition-all text-black"
                                    ...
                                    <button 
                                        ...
                                        class="p-2 bg-red-400 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none transition-all text-black"
                                        title="Từ chối"
                                    >
                                        <XCircle :size="18" />
                                    </button>
        ...
        <NeoConfirmModal 
            v-if="showConfirm"
            :title="actionType === 'APPROVE' ? 'Duyệt Mượn Sách' : actionType === 'REJECT' ? 'Từ Chối Mượn' : 'Xác Nhận Trả Sách'"
            :description="
                actionType === 'APPROVE' ? 'Xác nhận cho độc giả mượn cuốn sách này?' :
                actionType === 'REJECT' ? 'Từ chối yêu cầu mượn sách này? Sách sẽ được mở lại kho.' :
