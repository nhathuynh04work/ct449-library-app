...
const openCopyManager = (book: any) => {
...
<template>
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 border-4 border-black shadow-neo">
        ...
        <div v-if="isLoading" class="flex flex-col items-center justify-center py-20 bg-white border-4 border-black shadow-neo">
            <Loader2 :size="48" class="animate-spin text-black mb-4" />
            <p class="font-bold text-xl uppercase">Đang tải dữ liệu...</p>
        </div>

        <div v-else-if="isError" class="bg-red-100 border-4 border-black p-8 text-center shadow-neo">
        ...
        <div v-else class="bg-white border-4 border-black shadow-neo overflow-hidden">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-yellow-300 border-b-4 border-black text-black">
                            <th class="p-4 border-r-2 border-black font-black uppercase tracking-wide w-24">Mã</th>
                            <th class="p-4 border-r-2 border-black font-black uppercase tracking-wide">Thông tin Sách</th>
                            <th class="p-4 border-r-2 border-black font-black uppercase tracking-wide w-48">Tác giả & NXB</th>
                            <th class="p-4 border-r-2 border-black font-black uppercase tracking-wide w-48">Danh mục</th>
                            <th class="p-4 border-r-2 border-black font-black uppercase tracking-wide w-24 text-center">Bản sao</th>
                            <th class="p-4 font-black uppercase tracking-wide w-40 text-center">Thao tác</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y-2 divide-black">
                        ...
                        >
                            <td class="p-4 border-r-2 border-black align-top">
                                <span class="font-mono font-bold bg-black text-white px-2 py-1 text-sm block text-center">
                                    {{ book.maSach }}
                                </span>
                            </td>

                            <td class="p-4 border-r-2 border-black align-top">
                                <h4 class="font-black text-lg leading-tight mb-1">{{ book.tenSach }}</h4>
                            ...
                            <td class="p-4 border-r-2 border-black align-top text-sm">
                                <div class="mb-2">
                                    <p class="text-[10px] uppercase font-bold text-gray-500">Tác giả</p>
                                    <p class="font-bold truncate" :title="book.tacGia.map(t => t.tenTacGia).join(', ')">
                                        {{ book.tacGia.map(t => t.tenTacGia).join(", ") }}
                                    </p>
                                </div>
                                <div>
                                    <p class="text-[10px] uppercase font-bold text-gray-500">Nhà Xuất Bản</p>
                                    <p class="font-medium truncate">{{ book.nhaXuatBan.tenNhaXuatBan }}</p>
                                </div>
                            </td>
                            ...
                            <td class="p-4 border-r-2 border-black align-middle text-center">
                                <div class="flex items-center justify-center gap-1 font-black text-lg">
                            ...
                            <td class="p-4 align-middle text-center">
                                <div class="flex justify-center gap-2">
                                    <button 
                                        ...
                                        class="p-2 bg-blue-200 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none active:bg-blue-300 transition-all"
                        ...
                        <tr v-if="filteredBooks.length === 0">
                            <td colspan="6" class="p-12 text-center bg-gray-50">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="font-bold text-gray-500 text-lg">Không tìm thấy sách nào phù hợp.</p>
                                    <NeoButton v-if="searchQuery" @click="searchQuery = ''" variant="secondary" class="text-sm">
                                        Xóa bộ lọc
                                    </NeoButton>
                                </div>
                            </td>
            ...
            <div class="bg-white p-4 border-t-4 border-black flex justify-between items-center">
                ...
                <div class="flex gap-2">
                    <button class="px-3 py-1 border-2 border-black font-bold disabled:opacity-50 hover:bg-gray-100" disabled>Trước</button>
                    <button class="px-3 py-1 border-2 border-black bg-black text-white font-bold">1</button>
                    <button class="px-3 py-1 border-2 border-black font-bold hover:bg-gray-100">2</button>
                    <button class="px-3 py-1 border-2 border-black font-bold hover:bg-gray-100">Sau</button>
                </div>
            </div>
        </div>
        ...
        <NeoConfirmModal 
            v-if="deleteId"
            title="Xóa Sách"
            description="Bạn có chắc chắn muốn xóa cuốn sách này? Hành động này không thể hoàn tác và toàn bộ bản sao cũng sẽ bị xóa."
            variant="danger"
            @close="deleteId = null"
